...
ORDER BY passenger.PCardID,STime DESC -- 降序
);
select * from  digTakeRecord;
#6
...
select train.* from train,trainpass where train.SDate='2020-01-22' and
train.TID=trainpass.SID in
...
-- ?
select passenger.PCardID,passenger.PName,train.SDate from
train,taketrainrecord,diagnoserecord,passenger where
train.TID = taketrainrecord.TID and
passenger.PCardID = taketrainrecord.PCardID and
taketrainrecord.PCardID=diagnoserecord.PCardID and
diagnoserecord.DStatus=1 and
(train.TName,train.SDate,taketrainrecord.CarrigeID from
train,taketrainrecord,diagnoserecord WHERE
train.TID = taketrainrecord.TID and
taketrainrecord.PCardID=diagnoserecord.PCardID and
diagnoserecord.DStatus=1 GROUP BY train.TName,train.SDate,taketrainrecord.CarrigeID having count(*)=1) GROUP BY
