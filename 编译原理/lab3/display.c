...
{//处理基本表达式，参考文献[2]p82的思想
  ...
  if (T)
	...
	case EXP_ARRAY:
			 ...
			 rtn = searchSymbolTable(T->type_id);
                    ...
                    else
					{
                        ...
                        T->offset = (T->type == INT ? 4 : (T->type == FLOAT ? 8 : 1)) * getArray_width(T->ptr[0], symbolTable.symbols[rtn].array, 0); // 内存中偏移值
                        T->width = 0;   //未再使用新单元
	...
	case COMSTAR:
	case COMMOD:
	//printf("xsxssxssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss\n");
			if (T->ptr[0]->kind != ID ) {
