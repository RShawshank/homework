  ...
  if (T)
	...
	case ARRAY_DEFINE:
			...
			break;
				//=号
	case ASSIGNOP:
                if (T->ptr[0]->kind!=ID&&T->ptr[0]->kind!=ARRAY_DEFINE&&T->ptr[0]->kind!=ARRAY_CALL){
	...
	case DOUBLEADD:
				if(T->ptr[0])
				{
					...
					if(T->ptr[0]->type!=INT)
					{
						semantic_error(T->pos, "", "++语句的左值不是int型");
						break;
					}
					Exp(T->ptr[0]);
				T->type=T->ptr[0]->type;
	...
	case ARRAY_CALL:
			...
			if (rtn == -1){
				semantic_error(T->pos, T->ptr[0]->type_id, "数组没有定义");
				break;
			}
			else if (symbolTable.symbols[rtn].flag == 'F'){
				semantic_error(T->pos, T->ptr[0]->type_id, "是函数名，下标访问错误");
				break;
			}
			else if (symbolTable.symbols[rtn].flag == 'V'){
				semantic_error(T->pos, T->ptr[0]->type_id, "不是数组变量名，下标访问错误");
				break;
			}
			T->type = symbolTable.symbols[rtn].type;
			Exp(T->ptr[0]); 
...
void semantic_Analysis0(struct ASTNode *T) {
 ...
 void ArrayCall(struct ASTNode *T)
 {
	 if (T) {
		...
		{
			if(T->type_int<=0)
				semantic_error(T->pos,"","数组下标小于等于0");
		}
		else if(T->kind==FLOAT||T->kind==CHAR)
		{
			semantic_error(T->pos,"","数组下标不可为float型或是char型");
		}
		else if(T->kind==ARRAY_CALL)
		{
			Exp(T->ptr[0]);
			Exp(T->ptr[1]);
			ArrayCall(T->ptr[0]);
