...
%type  <ptr> program ExtDefList ExtDef  Specifier ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt StmList DefList Def DecList Dec Exp Args ArrayDec INDEX
//% token 定义终结符的语义值类型
...
//=assignop
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN FOR SWITCH CASE COLON DEFAULT COMADD COMSUB COMSTAR COMDIV COMMOD DOUBLEADD DOUBLESUB
%token BREAK CONTINUE 
/*以下为接在上述token后依次编码的枚举常量，作为AST结点类型标记*/
%token EXT_DEF_LIST EXT_VAR_DEF FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT IF_THEN IF_THEN_ELSE ARRAY_DEFINE ARRAY_CALL BLANK CONTINUE_ADD BREAK_ADD ARRAY_DEC
...
ExtDecList:  VarDec      {$$=$1;}       /*每一个EXT_DECLIST的结点，其第一棵子树对应一个变量名(ID类型(int、char、float)的结点),第二棵子树对应剩下的外部变量名*/
...
VarDec:  ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
	|ID ArrayDec {$$=mknode(1,ARRAY_DEFINE,yylineno,$2);strcpy($$->type_id,$1);}
	;
//数组定义：[index]
ArrayDec:LB INDEX RB {$$=mknode(1,ARRAY_DEC,yylineno,$2);strcpy($$->type_id,"ARRAY_DEC");} 
	...
	;
  //生成一个类型为INT的叶结点（度为0），指针赋值给Exp的属性$$，同时将INT的属性值$1（一个整常数）写到结点中type_int成员域保存。这里yylineno表示语法单元所在的行号。
...
Stmt:   Exp SEMI    {$$=mknode(1,EXP_STMT,yylineno,$1);}
      ...
      | BREAK SEMI		 {$$=mknode(0,BREAK_ADD,yylineno);strcpy($$->type_id,"BREAK");}
      | CONTINUE SEMI   {$$=mknode(0,CONTINUE_ADD,yylineno);strcpy($$->type_id,"CONTINUE");}
...
//add
 ...
 //     | DPLUS  Exp      {$$=mknode(1,DPLUS,yylineno,$2);strcpy($$->type_id,"DPLUS");}
  //    |   Exp DPLUS      {$$=mknode(1,DPLUS,yylineno,$1);strcpy($$->type_id,"DPLUS");}
      | DOUBLEADD  Exp   {$$=mknode(1,DOUBLEADD,yylineno,$2);strcpy($$->type_id,"DOUBLEADD");}
      | Exp DOUBLEADD      {$$=mknode(1,DOUBLEADD,yylineno,$1);strcpy($$->type_id,"DOUBLEADD");}
      | DOUBLESUB  Exp      {$$=mknode(1,DOUBLESUB,yylineno,$2);strcpy($$->type_id,"DOUBLESUB");}
      | Exp DOUBLESUB      {$$=mknode(1,DOUBLESUB,yylineno,$1);strcpy($$->type_id,"DOUBLESUB");}
      | ID Args RB{$$=$2;}
