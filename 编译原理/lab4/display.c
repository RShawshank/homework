...
int getArray_width(struct ASTNode *T, int *array, int index){
    if(T) {
        ...
        if(T->ptr[0]->kind == ID) {
            int SymbolTable = searchSymbolTable(T->ptr[0]->type_id);
            return (symbolTable.symbols[SymbolTable].const_int) * compute_arraywidth(array, index+1) + getArray_width(T->ptr[1], array, index+1);
        }
        else 
    ...
    switch (T->kind){
        case EXT_DEC_LIST:
               ...
               if (T->ptr[1]) {
                ...
                break;
        case ID:
            SymbolTable=fillSymbolTable(T->type_id,newAlias(),LEV,T->type,'V',T->offset);  //最后一个变量名
            if (SymbolTable==-1)
                semantic_error(T->pos,T->type_id, "变量重复定义");
            else T->place=SymbolTable;
            T->num=1;
            break;
        case ARRAY_DEC:
             array_index = 0;
            ...
            //对数组进行定义
            SymbolTable=fillSymbolTable(T->type_id,newAlias(),LEV,T->type,'A',T->offset);  //最后一个变量名
            if (SymbolTable==-1)
            ...
            //多维数组
        case ARRAY_LIST:
            SymbolTable = searchSymbolTable(array_name);
            if(SymbolTable == -1) {
            ...
            if(T->ptr[0]->type != INT) {
                ...
                symbolTable.symbols[SymbolTable].array[array_index] = T->ptr[0]->type_int;
                array_index++;
  ...
  if (T)
	...
	switch (T->kind) {
        ...
        //这个地方需要查看符号表，找到符号表中的位置，然后保存type
                    SymbolTable = searchSymbolTable(T->type_id);
            if (SymbolTable == -1)
                semantic_error(T->pos, T->type_id, "变量未定义");
            if (symbolTable.symbols[SymbolTable].flag == 'F')
                semantic_error(T->pos, T->type_id, "定义为函数名,类型不匹配");
            else if (symbolTable.symbols[SymbolTable].flag == 'A') 
                semantic_error(T->pos, T->type_id, "定义为数组名,类型不匹配");
            else {
                //opn1、opn2表示两个操作树，result表示运算结果
                opn1.kind = ID;
                strcpy(opn1.id, symbolTable.symbols[SymbolTable].alias);
                opn1.offset = symbolTable.symbols[SymbolTable].offset;
                opn2.kind = INT;
                opn2.const_int = 0;
                result.kind = ID;
		...
		case ARRAY_DEC:
                    SymbolTable = searchSymbolTable(T->type_id);
                    if(SymbolTable==-1)semantic_error(T->pos,T->type_id,"数组尚未定义");
                    else if(symbolTable.symbols[SymbolTable].flag=='F')
                        semantic_error(T->pos,T->type_id,"数组被定义成函数，类型不匹配");
                    else if(symbolTable.symbols[SymbolTable].flag=='V')
                        semantic_error(T->pos,T->type_id,"数组被定义成其他变量，类型不匹配");
                    else
                    {
                        T->place=SymbolTable;
                        T->type=symbolTable.symbols[SymbolTable].type;
                    }
                    break;
                   //条件转移
...
{//处理基本表达式，参考文献[2]p82的思想
  int SymbolTable,SymbolTable2,flag=0,num,width;
  ...
  if (T)
	...
	case ID:    //查符号表，获得符号表中的位置，类型送type
                ...
                if (symbolTable.symbols[SymbolTable].flag=='F')
                    ...
                    T->place=SymbolTable;       //结点保存变量在符号表中的位置
                    T->code=NULL;       //标识符不需要生成TAC
                    T->type=symbolTable.symbols[SymbolTable].type;
                    T->offset=symbolTable.symbols[SymbolTable].offset;
                    T->width=0;   //未再使用新单元
                    }
                break;
    case INT:   
	...
	case EXP_ARRAY:
			 ...
			 SymbolTable = searchSymbolTable(T->type_id);
                    ...
                    else
					{
                        ...
                        T->place = SymbolTable;       //结点保存变量在符号表中的位置
                        T->code = NULL;       //标识符不需要生成TAC
                        T->type = symbolTable.symbols[SymbolTable].type; // 标记ID类型
                        T->offset = (T->type == INT ? 4 : (T->type == FLOAT ? 8 : 1)) * (getArray_width(T->ptr[0], symbolTable.symbols[SymbolTable].array, 0)-1); // 内存中偏移值
                        T->width = 0;   //未再使用新单元
                        while(T0->kind==ARRAY_LIST){
                            ...
                            else if(symbolTable.symbols[SymbolTable].array[index] <= T0->type_int){
                                semantic_error(T->pos,"", "数组维度过大");
                                break;
                            }
                            index++;
                            T0=T0->ptr[1];
                ...
                if(T0->kind == ARRAY_LAST){
                    ...
                    if(T0->ptr[0]->kind == ID){
                        if(symbolTable.symbols[SymbolTable].array[index] <= symbolTable.symbols[T0->ptr[0]->place].const_int){
                            semantic_error(T->pos,"", "数组维度过大");
                        }
                    }
                    else{
                        if(symbolTable.symbols[SymbolTable].array[index] <= T0->ptr[0]->type_int){
                            semantic_error(T->pos,"", "数组维度过大");
                            }
                    }   
                    T->width += 4;
    ...
    case FUNC_CALL: //根据T->type_id查出函数的定义，如果语言中增加了实验教材的read，write需要单独处理一下
               ...
               //存放函数返回值的单数字节数
                ...
                while(T1 != NULL) {
                    count++;
                    num = SymbolTable;
                    if(symbolTable.symbols[SymbolTable+count].flag == 'P' && T1->type != symbolTable.symbols[SymbolTable+count].type){
                        semantic_error(T1->pos, "", "实参与形参类型不符");
                    }
                    T1 = T1->ptr[1];
                }
                param_num = symbolTable.symbols[SymbolTable].paramnum;
                if (count > param_num) {
                ...
                match_param(SymbolTable,T->ptr[0]);   //处理所有参数的匹配
                    //处理参数列表的中间代码
                T0=T->ptr[0];
                while (T0) {
                    result.kind=ID;  
                ...
                opn1.offset=SymbolTable;  //这里offset用以保存函数定义入口,在目标代码生成时，能获取相应信息
                result.kind=ID;   
				strcpy(result.id,symbolTable.symbols[T->place].alias);
                result.offset=symbolTable.symbols[T->place].offset;
	...
	if(T->ptr[0]->kind!=ID&& T->ptr[0]->kind != EXP_ARRAY)
			{
                ...
                opn1.const_int = 1;
                result.kind = ID;
		        SymbolTable = fill_Temp(newTemp(), LEV, INT, 'T', T->offset);
		        strcpy(result.id, symbolTable.symbols[SymbolTable].alias);
		        result.offset = symbolTable.symbols[SymbolTable].offset;
                T->code = merge(2, T->code, genIR(ASSIGNOP, opn1, opn2, result));
                ...
                opn1.type = INT;
       	    	opn1.offset = symbolTable.symbols[T->ptr[0]->place].offset;
                result2.kind = ID;
			        SymbolTable = fill_Temp(newTemp(), LEV, INT, 'T', T->offset+4);
			        strcpy(result2.id, symbolTable.symbols[SymbolTable].alias);
			        result2.offset = symbolTable.symbols[SymbolTable].offset;
...
{//对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
  ...
  if (T)
	...
	case FUNC_DEC:      //根据返回类型，函数名填写符号表
            SymbolTable=fillSymbolTable(T->type_id,newAlias(),LEV,T->type,'F',0);//函数不在数据区中分配单元，偏移量为0
            if (SymbolTable==-1){
            ...
            if (T->ptr[0]) { //判断是否有参数
                ...
                T->width=T->ptr[0]->width;
                symbolTable.symbols[SymbolTable].paramnum=T->ptr[0]->num;
                T->code=merge(2,T->code,T->ptr[0]->code);  //连接函数名和参数代码序列
                }
            else {symbolTable.symbols[SymbolTable].paramnum=0;
			T->width=0;}
            break;
	case PARAM_LIST:    //处理函数形式参数列表
            ...
            break;
	case  PARAM_DEC:
            SymbolTable=fillSymbolTable(T->ptr[1]->type_id,newAlias(),1,T->ptr[0]->type,'P',T->offset);
            if (SymbolTable==-1)
                semantic_error(T->ptr[1]->pos,T->ptr[1]->type_id, "参数名重复定义");
            else 
				T->ptr[1]->place=SymbolTable;
            T->num=1;       //参数个数计算的初始值
			...
			if (!strcmp(T->ptr[0]->type_id, "int")){
               ...
               // width=T->ptr[1]->type==INT?4:8;  //一个变量宽度
                while (T0) {  //处理所以DEC_LIST结点
                    ...
                    //DEC语句
                    if (T0->ptr[0]->kind==ID){
                        SymbolTable=fillSymbolTable(T0->ptr[0]->type_id,newAlias(),LEV,T0->ptr[0]->type,'V',T0->offset+T0->width);//此处偏移量未计算，暂时为0
                        if (SymbolTable==-1)
                            semantic_error(T0->ptr[0]->pos,T0->ptr[0]->type_id, "变量重复定义");
                        else 
							T0->ptr[0]->place=SymbolTable;
                        T->width+=width;
                        }
                    else if (T0->ptr[0]->kind==ASSIGNOP){
                            SymbolTable=fillSymbolTable(T0->ptr[0]->ptr[0]->type_id,newAlias(),LEV,T0->ptr[0]->type,'V',T0->offset+T0->width);//此处偏移量未计算，暂时为0
                            if (SymbolTable==-1)
